...
Last updated from: Price-filter and color-picker task status report (2026-08-18) — price-constrained queries ("jackets less than $40") no longer fail to the generic fallback: the real cause was a false positive in the Output Validator's price-fabrication check (the model echoing the customer's own stated budget back, e.g. "under $40", was being checked as if it were a claimed product price), now fixed by exempting threshold-qualified price mentions from that check. Product cards now render price/description text with a fixed, readable color independent of the admin-configured message-bubble theme, fixing a real clash where a light configured text color made card prices unreadable against the card's own fixed-white background. The three Appearance color fields are now real color-picker inputs (Magento's own shipped jQuery colorpicker, not a custom one), and any color left unset is now auto-computed to stay readable against whatever related color IS set, rather than falling back to a fixed default that might clash with it — manual values, when both halves of a pair are set, always win. Phase 1 (per architecture.md's roadmap table) remains functionally complete as of Task 11.
