...
- Incremental scheduling may publish only product ids. Validate the entire input before publishing anything, deduplicate and sort list updates deterministically, publish one id per message, and surface publisher failures as sanitized indexing exceptions.
