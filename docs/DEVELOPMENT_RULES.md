...
- Every ledger completion, retry, terminal transition, or lease-expiry recovery must prove exact product id, claimed generation, and lease token. Generation N completion must never erase generation N+1: scheduling during processing preserves the active lease and releases newer work only after the current claim finalizes or expires. Duplicate/stale queue wake-ups must be no-ops.
