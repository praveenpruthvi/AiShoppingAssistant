...
- Full assistant-index rebuilds must acquire the durable rebuild fence before opening a writer run, drain existing processing claims boundedly, renew and verify the exact owner token before alias activation, release the exact token after activation or abort, and wake bounded incremental recovery after the fence is released. Incremental claim transactions must lock/check the rebuild fence before locking a product work row.
