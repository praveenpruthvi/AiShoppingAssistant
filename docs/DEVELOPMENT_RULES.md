...
- Durable incremental work must flow through the ledger. Store only product id, generation, state, bounded attempts, next-attempt time, opaque lease token/expiry, sanitized error code, and timestamps. Never persist product content, snapshots, embeddings, provider payloads, URLs, credentials, raw exception text, traces, customer/cart data, or PII.
- Every ledger completion, retry, or terminal transition must prove exact product id, generation, and lease token. Generation N completion must never erase generation N+1, and duplicate/stale queue wake-ups must be no-ops.
