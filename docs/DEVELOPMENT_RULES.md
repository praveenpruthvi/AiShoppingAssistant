...
- Keep incremental writes/delete operations behind the exact store read alias after proving exactly one assistant-owned compatible physical target. Missing, mixed, foreign, malformed, or incompatible aliases must fail closed for retry/full rebuild; never use wildcard index access.
