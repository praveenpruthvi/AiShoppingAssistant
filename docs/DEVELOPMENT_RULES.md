...
- Keep every physical assistant index and alias in its own per-store namespace built by `IndexNamingServiceInterface` (`<prefix>_store_<store_id>_...`) with a validated store-scoped prefix; activation must remove only assistant-owned alias targets and abort must remove only run-owned unaliased indexes.
...
- Enrich embeddings through `EmbeddingEnrichmentServiceInterface` only, in bounded batches, correlating vectors positionally with the normalized documents and hashing the vector so unchanged content skips re-embedding.
- Never write index documents through the unavailable writer or accept partial run state: `OpenSearchProductDocumentWriter` requires the backend up and vector-capable before creating indexes, validates schema and store scope per batch, refreshes before alias activation, and treats alias activation as the single atomic commit point.
