...
- Product/category changes eventually enqueue affected entity IDs; observers and additional mview subscriptions are deferred. Milestone 2C2B1 declares Magento queue transport and stages a product-id-only publisher, but production row/list/mview scheduling remains fail-closed through `UnavailableIncrementalProductIndexScheduler` until durable recovery exists.
...
Hybrid retrieval and reranking are not yet implemented. The embedding fingerprint/dimension invalidation path is in place (a model or endpoint change that alters the fingerprint or dimensions invalidates the assistant index). Milestone 2C2A narrows local alias races for one incremental execution, and Milestone 2C2B1 adds staged Magento queue transport for product ids only; production activation, full-rebuild cutover coordination, and replay of missed/contended product events remain mandatory later Milestone 2C2B work.
