...
final class IncrementalProductIndexerTest extends TestCase
{
    ...
    public function testWritesEachEnabledStoreThroughItsOwnAlias(): void
    {
        ...
        $this->prepareTarget($this->scopes[2]);

        $this->buildIndexer()->process(42);

        self::assertSame(
            [
                ['index' => $this->alias($this->scopes[1]), 'id' => '1_42'],
                ['index' => $this->alias($this->scopes[2]), 'id' => '2_42'],
            ],
    ...
    public function testMissingProductDeletesStoreDocument(): void
    {
        ...
        self::assertSame([['index' => $this->alias($this->scopes[1]), 'id' => '1_42']], $this->client->deletedDocuments);
        self::assertSame([], $this->embeddingGeneration->calls);
    }
    ...
    public function testDeleteNotFoundIsIdempotent(): void
    {
        ...
        self::assertArrayNotHasKey('1_42', $this->client->documentSources[$this->alias($this->scopes[1])] ?? []);
    }

    public function testCompleteHashUnchangedIsNoOp(): void
    {
        $alias = $this->alias($this->scopes[1]);
        $this->client->documentSources[$alias]['1_42'] = $this->sourceFor($this->documents[1], [0.1, 0.2, 0.3, 0.4]);
    ...
    {
        $alias = $this->alias($this->scopes[1]);
        $this->client->documentSources[$alias]['1_42'] = $this->sourceFor(
        ...
        self::assertSame([0.1, 0.2, 0.3, 0.4], $this->client->documentSources[$alias]['1_42']['embedding']);
    }
    ...
    private function buildIndexer(): IncrementalProductIndexer
    {
        ...
        $snapshotProvider->method('load')->willReturnCallback(
            function (StoreScopeInterface $scope, IndexingConfigInterface $config, array $ids): ProductSnapshotBatch {
                if (($this->scenario[$scope->storeId()] ?? 'eligible') === 'missing') {
                    return new ProductSnapshotBatch([], [$ids[0]]);
                }

                return new ProductSnapshotBatch([$this->createMock(ProductSnapshotInterface::class)], []);
            }
        );
        ...
        $configSnapshot->method('matches')->willReturn(true);
    ...
    private function prepareTarget(StoreScope $scope): string
    {
        $naming = new IndexNamingService();
        $context = new RebuildRunContext(self::RUN_ID, ProductDocumentSchema::VERSION, [$scope], 1.0);
        ...
        $this->client->documentSources[$alias] = [];
